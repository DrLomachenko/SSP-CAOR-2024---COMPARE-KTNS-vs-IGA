...
std::vector<std::vector<int>> matrixFerramentasIGA;
...
int main(int argc, char *argv[])
{
	...
	//сделаем версию для быстрого считывания IGA и пока просто оставим пустым
	for (int i = 0; i < n; i++) {
		matrixFerramentasIGA.push_back(std::vector<int>());
	}
	//
	...
	for (unsigned j = 0; j < t; j++)
	{
		...
		for (unsigned i = 0; i < n; ++i)
		{
			int tooli = 0;
			cin >> tooli;
			//заполняем массив для IGA
			if (tooli == 1) {
				matrixFerramentasIGA[i].push_back(j);
			}
			//
	...
	*/
	...
	for (std::vector<std::pair<double, unsigned>>::const_iterator i = ranking.begin(); i != ranking.end(); ++i)
	{
		...
		{
			nTrocas = SwitchingCounter(processos, maquina-1, false);
	...
	for (std::vector<std::vector<int>>::const_iterator i = maquinas.begin(); i != maquinas.end(); ++i)
	{
		...
		std::cout << std::endl;
		std::cout << "Trocas: ";
		std::cout << SwitchingCounter(maquinaAux,maquina-1) << std::endl;
